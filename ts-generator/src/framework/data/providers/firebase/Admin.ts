...
export function firebase(appName: string, settings?: FirebaseSettings, cert?: string | admin.ServiceAccount) {
...
class FirebaseStore<T extends IDocument, LogEntry> extends DataStore<T, LogEntry> {
    ...
    private generateFilterQuery(query: admin.firestore.Query, criteria: IFilterCriteria) {
        return Object.keys(criteria)
            .reduce((prev: admin.firestore.Query, next: string) => {
    ...
    private generateOrderByQuery(query: admin.firestore.Query, orderBy: string[]) {
        return orderBy.reduce((prev: admin.firestore.Query, next) => {
            const [field, direction] = next.split(" ");
            return prev.orderBy(field, direction === "asc" ? "asc" : "desc");
        }, query);
    }
    private generateOptionsBasedQuery(query: admin.firestore.Query, options: ISearchOptions) {
        if (options.fields) {
    ...
    protected async read(filter?: () => IIndexable, options?: ISearchOptions): Promise<T[]> {
        let documents: undefined | admin.firestore.QuerySnapshot;
    ...
    protected async update(filter: () => IFilterCriteria, data: Partial<T>): Promise<T[]> {
        ...
        documents.docs.map(doc => {
            batch.set(doc.ref,
                Object.assign({
                } as admin.firestore.DocumentData, data), { merge: true });
        });
        await batch.commit();
